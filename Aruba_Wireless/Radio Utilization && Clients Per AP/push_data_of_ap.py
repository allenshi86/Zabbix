...
snmpwalk_ap_datas = os.popen("snmpwalk -v 2c -c momo 172.16.202.x %s" %prefix_ap_oid)
...
def remove_disabled_ap(dict_ap):
    for ap_name in dict_ap.keys():
        snmpwalk_status_datas = os.popen("snmpwalk -v 2c -c momo 172.16.202.x %s%s" % (prefix_ap_status_oid, dict_ap[ap_name]))
        ap_status = int(snmpwalk_status_datas.read().split(':')[-1])
        if ap_status != 1:
            dict_ap.pop(ap_name)
...
def trapper_channel_util(dict_ap):
    for ap_name in dict_ap.keys():
        try:
            snmpwalk_ap_radio0 = os.popen("snmpwalk  -v 2c -c momo 172.16.202.x %s%s.2" % (prefix_channel_oid, dict_ap[ap_name]))
            snmpwalk_ap_radio1 = os.popen("snmpwalk  -v 2c -c momo 172.16.202.x %s%s.1" % (prefix_channel_oid, dict_ap[ap_name]))
...
def check_double_5G(dict_ap):
    for ap_name in dict_ap.keys():
        datas = os.popen("snmpwalk -v 2c -c momo 172.16.202.x %s%s" % (prefix_double5G_oid, dict_ap[ap_name])).read()
...
def trapper_clients_ap(dict_ap):
    for ap_name in dict_ap.keys():
        radio0_datas = os.popen("snmpwalk -v 2c -c momo 172.16.202.x %s%s.2" % (prefix_radio_clients_oid, dict_ap[ap_name])).read()
        radio1_datas = os.popen("snmpwalk -v 2c -c momo 172.16.202.x %s%s.1" % (prefix_radio_clients_oid, dict_ap[ap_name])).read()
        ...
        os.system("/bin/zabbix_sender -z 172.16.7.x -vv -s office-aruba-bj-t2-11-ac-10 -k clients.radio0.[%s] -o %s" % (ap_name, clients_radio0))
        os.system("/bin/zabbix_sender -z 172.16.7.x -vv -s office-aruba-bj-t2-11-ac-10 -k clients.radio1.[%s] -o %s" % (ap_name, clients_radio1))
        os.system("/bin/zabbix_sender -z 172.16.7.x -vv -s office-aruba-bj-t2-11-ac-10 -k clients.[%s] -o %s" %(ap_name,clients_sum))
